...
GENERATABLE_JOB_STATUSES = {"APPROVED", "REJECTED", "CLOSED", "CERTIFIED"}
...
class ReportService:
    ...
    def generate(self, report_id: str, generated_by: str = "SYSTEM") -> Dict[str, Any]:
        ...
        try:
            ...
            report.update({"status":ReportStatus.REJECTED.value if rejected else ReportStatus.ISSUED.value,"report_status":ReportStatus.REJECTED.value if rejected else ReportStatus.ISSUED.value,"generation_status":"GENERATED","generated_at":output["generated_at"],"generated_by":generated_by,"template_name":output["template_name"],"template_version":output["template_version"],"html":output["html"],"job_snapshot":job,"instrument_snapshot":instrument,"approval_snapshot":output["context"]["approval"],"audit_snapshot":list(job.get("state_history") or []),"evidence_metadata":output["context"]["evidence"],"contains_demo_data":output["context"]["contains_demo_data"]})
            return {"success":True,"status_code":200,"data":self.repository.save(report)}
        except Exception:
            report.update({"generation_status":"FAILED","generation_error":"The report snapshot could not be assembled."}); self.repository.save(report)
    ...
    def dashboard_metrics(self) -> Dict[str, int]:
        ...
        return {"active_jobs":sum(state in {"CREATED","VALIDATED","TEST_PLAN_GENERATED","READY_FOR_TEST","IN_TESTING"} for state in states),"pending_reviews":sum(state in {"UNDER_REVIEW","TEST_COMPLETED"} for state in states),"failed_tests":sum(state == "REJECTED" for state in states),"retests":sum(str(job.get("job_type", "")).upper() == "RETEST" for job in jobs),"completed_jobs":sum(state in {"APPROVED","CLOSED","CERTIFIED"} for state in states),"verification_due":due,"reports_generated":sum(row.get("generation_status") == "GENERATED" for row in reports),"reports_pending_generation":sum(row.get("generation_status") != "GENERATED" for row in reports)}
